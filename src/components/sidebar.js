...
const Sidebar = () => {
  ...
  return (
    <div
      ...
      className="hs-overlay hs-overlay-open:translate-x-0 -translate-x-full transition-all duration-300 transform hidden fixed top-0 left-0 bottom-0 z-[60] w-96   lg:block lg:translate-x-0 lg:right-auto lg:bottom-0"
    >
      <div className=" w-64 px-6 py-7 bg-bg_body border-r border-gray-200 h-full overflow-y-auto scrollbar-y z-[65]">
        <div className="h-full w-full flex flex-col justify-between">
          ...
          <div>
            <ProfileLink
              ...
              openProfileMenu={setProfileMenuOpen}
            />
          </div>
        </div>
      </div>
      {profileMenuOpen && (
        <div
          className={`absolute bottom-10 ${
            profileMenuOpen ? "translate-x-72" : "translate-x-0"
          } transition-all duration-300 transform z-[62]`}
        >
          <ProfileMenu />
        </div>
      )}
